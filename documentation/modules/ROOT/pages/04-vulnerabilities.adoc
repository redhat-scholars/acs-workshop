...
We can use regular expresions as well. For instance, we are not usually interested in inactive images due to there is no deployment using it. For this reason we are not probably interested in check the vulneratilities for the inactive images so we can filter using **Deployment r/.+**. That means to list all the images where the deployment field is not empth, i.e. active images:
