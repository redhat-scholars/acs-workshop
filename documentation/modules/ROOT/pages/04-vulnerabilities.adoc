...
ACS scanner uses to fetch and update the vulnerability definitions. The first two are used by Central in ACS to fetch vulnerability details and collector modules.

http://definitions.stackrox.io/

collector-modules.stackrox.io is the other FQDN that will be used in online mode.