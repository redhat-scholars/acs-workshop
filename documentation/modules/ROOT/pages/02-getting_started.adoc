...
NOTE: You should check the specific version for you RHACS version to check memory/storage requirements ACS components and the sizing guidelines as well.
...
At this moment the ACS operator will be installing. It will take sometime to install, not much a few seconds.

Now we need to deploy central, so the first thing is to create the namespace:
...
Once the ACS Central is deployed we will need to add clusters to ACS. To add the cluster where we have deployed Central to ACS we need to download the https://mirror.openshift.com/pub/rhacs/assets/4.0.2/bin/Linux/roxctl[roxctl command.
...
[source,bash,subs="attributes+,+macros"]	
----	
roxctl -e <ACS CONSOLE URL>:443 central init-bundles generate acsdemo --insecure-skip-tls-verify --output-secrets /tmp/acsdemo.yaml --password <ACS ADMIN PASSWORD>
----
...
[source,bash,subs="attributes+,+macros"]	
----	
oc -n stackrox apply -f /tmp/acsdemo.yaml
----
